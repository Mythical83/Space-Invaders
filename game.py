...
def spawn_enemies():
  for i in range(0, random.randrange(5, 10)):
    enemies.append(Enemy(Actor("enemies/full_health", pos=(random.randrange(0, WIDTH - (WIDTH/(WIDTH/15))), random.randrange(0, 30))), random.randrange(1,3)))
...
def update_enemies():
  for enemy in enemies:
    ...
    if enemy.actor.right > WIDTH - (WIDTH/(WIDTH/15) - 5) - (WIDTH/(WIDTH/15)):
      enemy.actor.right = WIDTH - (WIDTH/(WIDTH/15) - 5) - (WIDTH/(WIDTH/15))
      enemy.direction = 1
      enemy.actor.bottom += 5
    for missile in missiles:

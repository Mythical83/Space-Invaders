...
def remove_health():
  ...
  health_to_remove = total_health/5
...
def update_enemies():
  for enemy in enemies:
    ...
    if enemy.actor.right > game_right - (WIDTH/(WIDTH/15)):
      enemy.actor.right = game_right - (WIDTH/(WIDTH/15))
      enemy.direction = 1
      enemy.actor.bottom += 5
    for laser in lasers:
...
def draw_health_bar():
  ...
  screen.draw.filled_rect(pg.Rect(game_right - 5, (HEIGHT - remaining_health)/2, WIDTH/(WIDTH/15), remaining_health), (0, 255, 0))

def draw_map(screen, w, h):
  ...
  for w in range(0, math.ceil(w/surface1.get_width())):
    screen.blit(surface1, (w*surface1.get_width(), h - (surface1.get_height() * 2) - 128))
    screen.blit(surface2, (w*surface1.get_width(), h - (surface1.get_height() * 2) - 96))
    screen.blit(surface3, (w*surface1.get_width(), h - (surface1.get_height() * 2) - 64))
    screen.blit(surface4, (w*surface1.get_width(), h - (surface1.get_height() * 2)))

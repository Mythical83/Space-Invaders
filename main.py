...
boss_total_health = 30
boss_remaining_health = 30
...
def remove_health_boss():
    global boss_remaining_health, boss_total_health
    boss_remaining_health -= boss_total_health/10
...
def spawn_enemies():
    ...
    for i in range(0, random.randrange(5 * level, 10 * level)):
        enemies.append(
            Enemy(Actor("enemies/full_health", pos=(random.randrange(0, game_right), random.randrange(0, 30))),
                  random.randrange(1, 3), 3, "common"))
...
def draw_health_bar():
    screen.draw.filled_rect(pg.Rect(game_right - 5, (HEIGHT - total_health) / 2, WIDTH / (WIDTH / 15), total_health),
                            (255, 0, 0))
    screen.draw.filled_rect(
        pg.Rect(game_right - 5, (HEIGHT - remaining_health) / 2, WIDTH / (WIDTH / 15), remaining_health), (0, 255, 0))

def draw_boss_bar():
  screen.draw.filled_rect(pg.Rect())
